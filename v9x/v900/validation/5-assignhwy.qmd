...
Validation of final highway assignment was done comparing model outputs from the final loaded networks with observed data. The comparisons were done with volumes and speeds.
...
```{python}
allveh_valid = pd.read_csv("data/5-assignhwy/wf-validation-09-18-2023-AllVeh.csv")
...
![Segment-Level Model vs Observed Volume Comparison by Vehicle Type](_pictures/5-assignhwy-map.png){#fig-map}
...
gdf_d = gpd.GeoDataFrame(df1, geometry=geometry_d, crs='EPSG:4326')
...
dfa['Speed']=(dfa[' ODDistance']/ dfa[' ODTime'])*3600/1609.34
...
dfb['Speed']=(dfb[' ODDistance']/ dfb[' ODTime'])*3600/1609.34
dfa=pd.concat([dfa,dfb])
dfx=dfa[['O_TAZID',' D_TAZID','Period','Speed']]
...
# Speed Result
xx=dfx.groupby(['O_TAZID', ' D_TAZID','Period']).agg({'Speed': 'median'}).unstack().Speed.rename_axis([None], axis=1).reset_index()
#display(xx)

# Speed Result Count
xx2=dfx.groupby(['O_TAZID', ' D_TAZID','Period']).agg({'Speed': 'count'}).unstack().Speed.rename_axis([None], axis=1).reset_index()
#display (xx2)
```

```{python}
_path = "//10.0.0.114/ModelAce-D/1 - TDM/3 - Model Dev/1 - WF/1 - Official Release/v9x/v9.0/WF TDM v9.0 - official/Scenarios/BY_2019/5_AssignHwy/5_FinalNetSkims/"
...
def extract_values_from_omx(omx_file_path, column_name):
    ...
    with omx.open_file(omx_file_path, 'r') as f:
        ivt_matrix = f['GP_IVT'][:] / 60.0  # Convert minutes to hours
...
for omx_file, column_name in zip(omx_files, columns):
    xx = extract_values_from_omx(_path + omx_file, column_name)
...
obdSpd = xx.melt(id_vars=('O_TAZID',' D_TAZID'),var_name='Period',value_vars=('obsAM','obsMD','obsPM','obsEV'), value_name='spdObs')
obdSpd['Period'] = obdSpd['Period'].str[3:]
#display(obdSpd)

modSpd = xx.melt(id_vars=('O_TAZID',' D_TAZID'),var_name='Period',value_vars=('modAM','modMD','modPM','modEV'), value_name='spdMod')
modSpd['Period'] = modSpd['Period'].str[3:]
#display(modSpd)

dfSpd = pd.DataFrame.merge(obdSpd, modSpd, on=('O_TAZID',' D_TAZID','Period'))
#display(dfSpd)
```

```{python}
ojs_define(speed = dfSpd) 
