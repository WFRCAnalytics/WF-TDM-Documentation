...
fnObs        = 'A:/0 - Source Data/1 - Surveys/2019 UTA Transit On-Board Survey/2019 Final Weighted UTA OD Data - 2022-07-07.csv'
...
The motorized/non-motorized model shares are within 0.1% of the observed data for all time periods and all trip purposes and with 0.5% for any combination of time period and purpose. The model transit mode shares are also close to observed with all modes for all time periods and trip purposes with 5%.
...
//|fig-cap: "Model vs Observed Mode Shares by Mode Group, Time Period, and Trip Purpose"
emptycell = 1
```
...
```{python}
dbfTdmPaRoute=pd.DataFrame(DBF(fnTdmPaRoute, load=True))
dbfTdmPaRoute

dbfTdmPaRouteLnkMdBrdMd = pd.melt(dbfTdmPaRoute, id_vars=['MODE','NAMEID','NAME'], value_vars=['DY_4_XITB','DY_5_XITB','DY_6_XITB','DY_7_XITB','DY_8_XITB','DY_9_XITB'])
dbfTdmPaRouteLnkMdBrdMd[nameModeTransit+' - modeTDM'] = pd.to_numeric(dbfTdmPaRouteLnkMdBrdMd['variable'].str.get(3))
```

```{python}
# TDM Boardings - BoardMode
dfTdmBrd = dbfTdmPaRouteLnkMdBrdMd.groupby(['MODE',nameModeTransit+' - modeTDM'],as_index=False).agg({'value': [np.sum]})
...
#calculate difference
dfBrdLnkTrip['Diff'] = dfBrdLnkTrip[nameBoardMod] - dfBrdLnkTrip[nameBoardObs]
dfBrdLnkTrip['% Diff'] = dfBrdLnkTrip['Diff'] / dfBrdLnkTrip[nameBoardObs]
```
...
dfTxRatio = dfTxRatio[[nameModeTransit, nameTxRatioMod, nameTxRatioObs, 'Diff', '% Diff']]
```
...
#calculate difference
dfBrdMdSrvy['Diff'] = dfBrdMdSrvy[nameBoardMod] - dfBrdMdSrvy[nameBoardObs]
dfBrdMdSrvy['% Diff'] = dfBrdMdSrvy['Diff'] / dfBrdMdSrvy[nameBoardObs]
```

```{python}
df1_Trips = (dfTrips
    .reset_index()
    .rename(columns={nameModeTransit: 'Mode', nameTripsMod:'Model', nameTripsObs:'Observed', '% Diff': 'PercentDiff'}))
...
df1_Boardings_LinkedTrip = (dfBrdLnkTrip
    .reset_index()
    .rename(columns={nameModeTransit: 'Mode', nameBoardMod:'Model', nameBoardObs:'Observed', '% Diff': 'PercentDiff'}))
...
df1_TxRatio = (dfTxRatio
    .reset_index()
    .rename(columns={nameModeTransit: 'Mode', nameTxRatioMod:'Model', nameTxRatioObs:'Observed', '% Diff': 'PercentDiff'}))
...
df1_Boardings_ModeSurveyed = (dfBrdMdSrvy
    .reset_index()
    .rename(columns={nameModeTransit + ' - Surveyed': 'Mode', nameBoardMod:'Model', nameBoardObs:'Observed', '% Diff': 'PercentDiff'}))
...
# add diff column
dfboardings_obj['Difference'] = dfboardings_obj['Model'] - dfboardings_obj['Observed']
