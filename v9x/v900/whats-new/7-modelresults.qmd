...
The comparison between daily volumes at the segment level can be found in @fig-volume-comparison for 2019 and 2050. Decreases in volume in version 9 compared to version 8 are shown in blue, while increases are shown in red.
...
Model Daily Volumes Comparison - All Vehicles (version 9 vs version 8)
:::
...
Model Daily Volumes Comparison - Trucks (version 9 vs version 8)
:::
...
dfRouteSummaryWithScenarioData = pd.read_csv("data/intermediate/routesummaryresults.csv")
dfTransitSummaryPlotData = pd.read_csv("data/intermediate/transitsummaryresults.csv")
dfV8RouteSummaryResults = pd.read_csv("data/intermediate/v832-boardings.csv")
```
...
def update_plot_stackedarea_boardings(df, tdmVersionWithDate, scenarioGroup):
...
update_plot(
    ...
    ['RTP'], 
    ['4: Transit'], 
    ['HBC', 'HBO', 'HBS', 'HBW', 'NHB', 'ALL'],  #'ALL' is just for v832
    ['Pk','Ok', 'DY'] #'DY' is just for v832
)
```
...
#| fig-cap: "Transit Trips Share by Mode - Version 9"
#| cap-location: margin
...
#| fig-cap: "Transit Trips Share by Mode - Version 8"
#| cap-location: margin
...
```{python}
#| label: fig-brd-9
#| fig-cap: "Transit Boardings Share by Mode - Version 9"
#| cap-location: margin

update_plot_stackedarea_boardings(dfRouteSummaryWithScenarioData, modelNew, 'RTP')
```
...
```{python}
#| label: fig-brd-832
#| fig-cap: "Transit Boardings Share by Mode - Version 8"
#| cap-location: margin

update_plot_stackedarea_boardings(dfV8RouteSummaryResults, modelOld, 'RTP')
```
