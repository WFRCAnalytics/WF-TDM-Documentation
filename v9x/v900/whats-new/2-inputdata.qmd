...
### Control Totals
::: {.content-hidden when-format="html"}
Updates to the county control totals were made based on projections from the Gardner Policy Institute. The v8.3.2 model used a base year of 2015 for controls totals. v9 uses a base year of 2019. @fig-pdf-res-control and @fig-pdf-emp-control shows the differences between 2020 and 2060 in residential and employment control totals for the Wasatch Front as a whole.  In addition @fig-pdf-bus-rat-control shows control total ratios for the Wasatch Front as a whole between the same years. For specific county by county breakdowns, please visit the [online documentation](https://wfrc.org/wftdm-docs/v9x/v900/whats-new/2-inputdata.html). 

Although not displayed here, it is important to note that Utah County showed the largest decrease in control totals in the v9 model as compared to v8.3.2 with 107,937 lower population, 21,799 fewer households, and 47,977 fewer jobs (all employment). The largest job decreases are in office employment (14,923), government/education employment (16,982), and other employment (18,194). Utah County also has a significant drop in household size relative to other counties.

![Residential Control Total: Wasatch Front](_pictures/pdf-residential-control.png){#fig-pdf-res-control}

![Employment Control Total: Wasatch Front](_pictures/pdf-employment-control.png){#fig-pdf-emp-control}

![Control Total Ratios: Wasatch Front](_pictures/pdf-ratio-control.png){#fig-pdf-bus-rat-control}

:::

::: {.content-visible when-format="html"}
Updates to the county control totals were made based on projections from the Gardner Policy Institute. The v8.3.2 model used a base year of 2015 for controls totals. v9 uses a base year of 2019. @fig-county-control-total-differences-by-category shows the 2050 differences in county control totals for any given socioeconomic category. Further trends for the selected socioeconomic category for a specific county is shown in @fig-county-control-trends-by-county-by-category.
...
chart3 = DivergingBarChart(filtered_dataCTDiff50, {
    ...
    yDomain: ['Weber','Davis','Salt Lake','Utah']
})
```

```{ojs}
viewof selectCounty = Inputs.select(new Map(['Weber', 'Davis', 'Salt Lake', 'Utah'].map(county => [county, county])), {value: 'Salt Lake', label: 'County Trends: '});
```
