...
fnTaz832In    = r"A:\1 - TDM\3 - Model Dev\1 - WF\1 - Official Release\v8.3.2\WF TDM v8.3.2 - 2023-02-28\1_Inputs\1_TAZ\TAZ.shp"
fnTaz900In    = r"A:\1 - TDM\3 - Model Dev\1 - WF\2 - Sandbox\v9.0Beta\WF TDM v9.0 - 2023-04-14\1_Inputs\1_TAZ\TAZ.shp"
fnK12EnrollIn = r"A:\1 - TDM\3 - Model Dev\1 - WF\2 - Sandbox\v9.0Beta\WF TDM v9.0 - 2023-04-28\1_Inputs\2_SEData\_ source - HBSch Enroll & Med Inc\K-12 Enrollment\k12_schools_enrollment\k12_schools_enrollment.shp"
...
writeGeoJson(fnTaz832In   , "data/taz832.geojson"   , ['TAZID','CO_TAZID','geometry'])
writeGeoJson(fnTaz900In   , "data/taz900.geojson"   , ['TAZID','CO_TAZID','REMM','geometry'])
...
if not os.path.isfile('data/tazmedincome.geojson'):
  gdfTaz900 = gpd.read_file("data/taz900.geojson")
  dfMedIncome = pd.read_csv(r"A:\1 - TDM\3 - Model Dev\1 - WF\2 - Sandbox\v9.0Beta\WF TDM v9.0 - 2023-04-28\1_Inputs\2_SEData\_ source - HBSch Enroll & Med Inc\Median Income & VOT\for SE file -- TAZ Median Income - 2022-03-17.csv")
  gdfTazMedIncome = pd.DataFrame.merge(gdfTaz900,dfMedIncome,on='CO_TAZID',how='left')
...
geojsonTaz832    = FileAttachment("data/taz832.geojson"   ).json()
geojsonTaz900    = FileAttachment("data/taz900.geojson"   ).json()
...
mapTaz = {
  ...
  let map = L.map(container).setView([40.7608, -111.8910], 8);
  ...
  let lyrTAZ832 = L.geoJson(geojsonTaz832, { weight: 3.00, color: lcolors[0]}).addTo(map);
  let lyrTAZ900 = L.geoJson(geojsonTaz900, { weight: 0.75, color: lcolors[1]}).addTo(map);
...
mapTazRemmSpace = {
  ...
  let lcolors = ['#00FF7F'   ,'#BD0026'       ]
  let llabels = ['REMM Space','Non-REMM Space'];
  
  let lyrTAZ900 = L.geoJson(geojsonTaz900, {
                            style: function(feature) {
                              var d = feature.properties.REMM;
                              return d==1 ? {color: lcolors[0], opacity:0.95} :
                                     d==0 ? {color: lcolors[1], opacity:0.95} : 
...
### Control Totals
Control total updates were made to the *ControlTotal_SE_AllCounties.csv* and *ControlTotal_WorkAtHome.csv* files. The source worksheet for these input files are found in the *-\_Source - ControlTotal_SE - 2022-08-31.xlsx* file. Other files in this directory were removed.


```{python}
import pandas as pd

fnConTot_v832 = r"A:\1 - TDM\3 - Model Dev\1 - WF\1 - Official Release\v8.3.2\WF TDM v8.3.2 - 2023-02-28\1_Inputs\2_SEData\_ControlTotals\ControlTotal_SE_AllCounties.csv"
fnConTot_v900 = r"A:\1 - TDM\3 - Model Dev\1 - WF\2 - Sandbox\v9.0Beta\WF TDM v9.0 - 2023-04-28\1_Inputs\2_SEData\_ControlTotals\ControlTotal_SE_AllCounties.csv"


dfConTotv832 = pd.read_csv(fnConTot_v832)
dfConTotv832['modelVersion'] = 'v8.3.2'

dfConTotv900 = pd.read_csv(fnConTot_v832)
dfConTotv900['modelVersion'] = 'v9.0.0'

dfConTot = pd.concat([dfConTotv832,dfConTotv900])
display(dfConTot)
